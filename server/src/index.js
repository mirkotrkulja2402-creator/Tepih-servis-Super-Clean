...
app.get("/api/health", async (_req,res)=>{
  let db="local-dev";
  if(pool){try{await pool.query("select 1");db="postgres-ok"}catch(e){db="postgres-error"}}
...
app.get("/api/config", async (_req,res)=>{
  res.json({
    ...
    deliveryPrice:5,
    modules:["Kupci","Cjenovnik","Narudžbe","Mjerenje","Računi","Blagajna","Izvještaji","Ruta","GARI","QR KOD","Administrator"]
  });
});
