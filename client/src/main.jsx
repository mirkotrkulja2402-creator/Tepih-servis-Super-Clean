...
function CashDesk({db,commit}){
  ...
  return <Module title="Blagajna" subtitle="Evidencija svih računa i naplate bez brisanja dokaza.">
    ...
    <Table columns={[["no","Račun"],["customer","Kupac"],["total","Iznos"],["payment","Način"],["paid","Plaćeno"],["audit","Evidencija"]]}
      rows={db.invoices.map(i=>{const p=db.payments.find(x=>x.invoiceId===i.id);return {...i,total:money(i.total),paid:<input type="checkbox" checked={!!p?.paid} onChange={()=>toggle(i)} onClick={e=>e.stopPropagation()}/>,audit:p?.by?`${p.by} • ${new Date(p.at).toLocaleString("bs-BA")}`:""} )}/>
  </Module>
}
